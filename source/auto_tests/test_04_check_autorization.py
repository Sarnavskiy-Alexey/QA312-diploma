...
def check_autorization(browser_data: webdriver.Chrome | webdriver.Edge | webdriver.Firefox):
    ...
    if browser_data.current_url != GL_ARGUMENTS['url']:
        logging.error(f'{browser_data.current_url} - url did not changed on home')

    # 7, 8
    time.sleep(1)
    browser_data.get(GL_CONST_REFERENCES['Logout'])
    time.sleep(1)
    if browser_data.current_url != GL_CONST_REFERENCES['Signup / Login']:
        logging.error(f'{browser_data.current_url} - url did not changed on signup/login')
    else:
        login_form = browser_data.find_element(By.CLASS_NAME, 'login-form')
        ...
        if warning_paragraph.is_displayed() and warning_paragraph.text == 'Your email or password is incorrect!':
            logging.error(f"{browser_data.current_url} - 'Your email or password is incorrect!' paragraph is found!")
...
def test_04_authorization_chrome(browser_data_chrome, caplog):
    time.sleep(2)
    caplog.set_level(logging.INFO)
    check_autorization(browser_data_chrome)
    with open('source/auto_tests/tests/chrome/test_04.log', mode='wt') as f:
        for line in caplog.text:
            f.write(line)


def test_04_authorization_edge(browser_data_edge, caplog):
    time.sleep(2)
    caplog.set_level(logging.INFO)
    check_autorization(browser_data_edge)
    with open('source/auto_tests/tests/edge/test_04.log', mode='wt') as f:
        for line in caplog.text:
            f.write(line)


def test_04_authorization_firefox(browser_data_firefox, caplog):
    time.sleep(2)
    caplog.set_level(logging.INFO)
    check_autorization(browser_data_firefox)
    with open('source/auto_tests/tests/firefox/test_04.log', mode='wt') as f:
        for line in caplog.text:
            f.write(line)
